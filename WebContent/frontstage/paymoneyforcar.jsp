...
<html lang="zh-CN">
	...
	<body class="pb12 fadeIn animated">
		...
		<select id="packId" name="packId">
			...
			<option value="${pack.packId}">${pack.packName} 时长:${pack.packTime}/月</option>
			</c:forEach>
		</select>
	...
	function addMyCar(){
		...
		if(carId!=null&&carId!=""&&packId!=null&&packId!=""){
		$.ajax({
			type:"post",
			url:"<%=request.getContextPath()%>/owerHandler/payMoneyForCar.action",
			...
			error:function(){
				window.loaction="<%=request.getContextPath()%>/frontstage/error.jsp";
			}
		});
		}
	};
