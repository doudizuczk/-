...
window.onload=function(){
  ...
  //注入规则明细
    <c:forEach items="${requestScope.ruleList}" var="t">  
    	vu.myData.push({
	...
	$("#update").click(function(){
		...
		$.ajax({
			type : "post",
			url : "<%=request.getContextPath()%>/ruleHandler/updateRule.action",
			data : {"ruleName" : $("#ruleName").val(),"ruleDetail":str,"sequence":"${requestScope.ruleList[0].sequence }"},
			success : function(data) {
				if (data == '1') {
					alert("添加成功！新入库车辆将使用此规则");
...
<body>
	<form id="createForm">
		<table class="table table-striped table-hover">
			<tbody>
				<tr>
					<th>规则名称：</th>
					<td><input type="text" name=ruleName id="ruleName" value="${requestScope.ruleList[0].ruleName }" placeholder="请输入规则名称...">
    ...
    <button id="update" class="btn btn-primary btn-sm">保存提交</button>
</body>
</html>