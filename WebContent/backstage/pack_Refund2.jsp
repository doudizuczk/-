 		 ...
 		 if(transact==null){//新办
	 		...
	 		if(packId==transact.tran.PACK_ID){
				...
				$("#packLabel").html("(续费)"); //回填显示信息
	 			...
	 			PackTranPyte=3;
	 			if(owerStates==1){//有账号
	 	 			alert($("#owerMon").text())//余额
	 	 			alert(newPackAtt.PACK_COST)//费用
	 	 			alert(transact.money)//退费
	 	 				if($("#owerMon").text()+transact.money<newPackAtt.PACK_COST){//判断余额是否不足
	 	 					$("#part1").attr("disabled", true);//余额支付不可选
	 	 		    		$("#PayType").html("(余额不足)"); //回填显示信息
	 	 				}else{
